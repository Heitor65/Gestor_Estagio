...
class Aluno(models.Model):
    ...
    periodo = models.PositiveIntegerField(validators=[validar_periodo])
    curso = models.CharField('Curso', max_length=100, choices=CURSOS_CHOICES)
    ...
    def __str__(self):
        return f"{self.usuario.username} | Curso: {self.curso}"


    
class Secretaria(models.Model):
    usuario = models.OneToOneField(Usuario, on_delete=models.CASCADE, db_column='matricula', verbose_name="Usuário")
    carteira_de_trabalho = models.IntegerField(default=0)
...
class Empresa(models.Model):
    ...
    def __str__(self):
        return f"{self.razao_social} - CNPJ: {self.cnpj} | {self.cidade}/{self.uf}"
...
class Estagio(models.Model):
    ...
    def __str__(self):
        ...
        return f"Estágio: {self.tce.aluno.usuario.username} | Empresa: {self.empresa.razao_social} | {self.dtinicio.strftime('%d/%m/%Y')} → {dtfim_str}"
